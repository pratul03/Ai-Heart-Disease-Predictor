  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="secondary" className="mr-5">{label}</Button>
      </DialogTrigger>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="space-y-1">
                  ...
                  <Input id="username" defaultValue="john123@xyz.com" onInput={(e)=> {
                    setSignin({...signin, email: e.currentTarget.value})
                  }}/>
                </div>
                <div className="space-y-1">
                  <Label htmlFor="password">Password</Label>
                  <Input id="password" type="password" onInput={(e) => {
              ...
              <CardFooter>
                <Button variant={"secondary"} onClick={async() => {
                  toast.promise(
                    axios.post("http://localhost:8080/api/auth/signin", signin),
                    {
                      loading: "Signing in...",
                      success: (response) => {
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="space-y-1">
                  ...
                  <Input id="name" defaultValue={"John Doe"} onInput={(e) => {
                    setSignup({...signup, name: e.currentTarget.value})
                  }}/>
                </div>
                <div className="space-y-1">
                  <Label htmlFor="email">Email</Label>
                  <Input id="email" type="email" defaultValue={"john123@xyz.com"} onInput={(e) =>{
              ...
              <CardFooter>
                <Button variant={"secondary"} onClick={async() => {
                  toast.promise(
                    axios.post("http://localhost:8080/api/auth/signup", signup),
                    {
                      loading: "Signing up...",
                      success: (response) => {
                        localStorage.setItem("token", response.data.token);
